...
class OsPlacesService {
  async boot (options) {
    ...
    this.apiUrlPrefix = 'https://api.ordnancesurvey.co.uk/places/v1/addresses/'
  }
  ...
  async searchAddress (options = {}) {
    ...
    if (this.apiKey === 'DISABLED') {
      console.log('OS_PLACES_API_KEY is not set')
    ...
    const url = lookupType === 'uprn'
      ? `${this.apiUrlPrefix}uprn?key=${this.apiKey}&uprn=${encodeURIComponent(query)}&dataset=DPA,LPI`
      : `${this.apiUrlPrefix}find?key=${this.apiKey}&query=${encodeURIComponent(query)}&offset=${offset}&maxresults=${limit}&dataset=DPA,LPI`
