...
class OsPlacesService {
  ...
  async searchAddress (options = {}) {
    ...
    try {
      const { data } = await axios.get(`${this.apiUrl}&query=${query}&offset=${offset}&maxresults=${limit}&dataset=DPA,LPI`)

      return { results: data.results, totalHits: data.header.totalresults }
    } catch (err) {
