...
class OsPlacesService {
  ...
  async searchAddress (options = {}) {
    ...
    if (query === '') {
      ...
      await this.addressLookupReceiptModel.create({ query, offset, limit, status: 'FAILED', errorMessage: 'Cannot search address without a query string', totalHits: 0 })
    ...
    try {
      ...
      await this.addressLookupReceiptModel.create({ query, offset, limit, status: 'SUCCEEDED', totalHits })
      ...
      await this.addressLookupReceiptModel.create({ query, offset, limit, status: 'FAILED', errorCode: err.response.status, errorMessage: err.response.statusText, totalHits: 0 })
